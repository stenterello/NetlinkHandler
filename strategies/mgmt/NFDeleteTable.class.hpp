...
class	NFDeleteTable : public NFStrategy {
	protected:
	public:
		explicit					NFDeleteTable(const char* table_name) : NFStrategy(table_name) {};

		struct msghdr&				forgeRequest(struct sockaddr_nl& dst) override {
			_nlmsgptr->nlmsg_len = NLMSG_LENGTH(sizeof(struct nfgenmsg));
			_nlmsgptr->nlmsg_type = (NFNL_SUBSYS_NFTABLES << 8) | NFT_MSG_DELTABLE;
