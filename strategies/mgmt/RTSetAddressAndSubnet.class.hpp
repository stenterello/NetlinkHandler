...
class	RTSetAddressAndSubnet : public RTMgmtStrategy {
	private:
		const char*		_interface;
		const char*		_ip;
		const uint8_t 	_subnet;
	public:
		RTSetAddressAndSubnet(const char* iface, const char* ip, uint8_t subnet): RTMgmtStrategy(),
																					_interface(iface),
																					_ip(ip),
																					_subnet(subnet){};
		...
		struct msghdr&				forgeRequest(struct sockaddr_nl& dst) override {
			...
			_nlmsgptr->nlmsg_type = RTM_NEWADDR;
