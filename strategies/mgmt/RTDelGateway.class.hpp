...
class	RTDelGateway : public RTMgmtStrategy {
	protected:
		const char*		_interface;
		const char*		_ip;
		int 			_ifaceIndex{};
	public:
		RTDelGateway(const char* interface, const char* ip): _interface(interface), _ip(ip) {};

		struct msghdr&				forgeRequest(struct sockaddr_nl& dst) override {
			char addr[INET_ADDRSTRLEN];

			_nlmsgptr->nlmsg_len = NLMSG_LENGTH(sizeof(struct rtmsg));
			_nlmsgptr->nlmsg_type = RTM_DELROUTE;
			...
			std::cout << "Gateway addr to delete " << _ip << std::endl;
			...
			std::cout << "interface on which gateway will be deleted " << _interface << std::endl;
